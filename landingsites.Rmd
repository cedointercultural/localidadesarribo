...
fishery.offices <- sf::st_read(here::here("data-raw","conapesca_offices","offices_wgs84.shp")) %>%   sf::st_transform(crs = 4326) %>% 
  sf::st_centroid() %>% 
  mutate(FED_ENTITY = dplyr::if_else(FED_ENTITY == "BAJA CALIFORNIA NORTE", "BAJA CALIFORNIA", FED_ENTITY)) %>% 
    keep_when(FED_ENTITY %in% coast.selecc)

fishery.names <- fishery.offices %>% 
  sf::st_drop_geometry() %>%
  select(FED_ENTITY, OFFICE) %>%
  mutate(NOM_ENT = FED_ENTITY, NOM_LOC = OFFICE)
 
fishery.correct.loc <- fishery.names %>%
  ...
  dplyr::left_join(locs.census, by = c("NOM_LOC","NOM_ENT")) 


fishery.distance <- fishery.offices %>% 
  sf::st_distance(georef.locs.sp) %>% 
  sf::st_as_sf() %>% 
  sf::st_set_crs(4326) %>% 
  sf::st_set_geometry(NULL) %>% 
  dplyr::mutate(dist_fishery = .) %>% 
  dplyr::bind_cols(georef.locs) %>% 
  dplyr::select(NOM_ENT, NOM_LOC, deci_lat, deci_lon, dist_fishery) %>% 
  dplyr::arrange(NOM_ENT, NOM_LOC) %>% 
  readr::write_csv(here::here("outputs","georef_locations.csv"))
