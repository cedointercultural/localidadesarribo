...
permit.data <- lapply(no.permitfiles, extract_permits, permit.files, inegi.cost.comm.all, coast.selecc, locs.pesca)

permit.data.rev <- permit.data %>% 
  dplyr::bind_rows() %>% 
  distinct(rnp_code, NOM_ENT, CVE_LOC, NOM_LOC, deci_lat, deci_lon)
...
locs.inegi <- googlesheets4::read_sheet("1Nww_0aSf2yQv9YqK8LqmJmIM2wwpHZCSpG1NcGfIfHQ", sheet = "busqueda_inegi") %>% 
  dplyr::rename(NOM_LOC=NOM_LOC_ORIGINAL, deci_lat=latitud, deci_lon=longitud) %>% 
...
locs.google <- googlesheets4::read_sheet("1Nww_0aSf2yQv9YqK8LqmJmIM2wwpHZCSpG1NcGfIfHQ", sheet = "busqueda_google") %>% 
  dplyr::rename(NOM_LOC=NOM_LOC_ORIGINAL, deci_lat=lat, deci_lon=lon) %>% 
  keep_when(utilizar == 1) %>% 
  select(NOM_ENT, NOM_LOC, deci_lat, deci_lon) %>% 
  mutate(deci_lon=as.numeric(deci_lon), deci_lat=as.numeric(deci_lat))

locs.pesca.pacifico <- googlesheets4::read_sheet("1d4KPHneHWikkXQ1ELEC5D16OFMu_2txz") %>% 
