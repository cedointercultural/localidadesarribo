...
get_distance <- function(thisloc, fishery.offices){
  ...
  this.distance <- this.point %>% 
    ...
    dplyr::distinct(fshry_f, NOM_ENT, CVE_LOC, NOM_MUN, NOM_LOC, CVE_ENT, CVE_MUN, distance)
  
  this.point.dis <- this.point.tbl  %>% 
    mutate(fishery_office = this.distance$fshry_f, fishery_office_ent = this.distance$NOM_ENT, distance = this.distance$distance) %>% 
